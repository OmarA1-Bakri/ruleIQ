...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Brighten and apply purple/silver theme
    vec3 color = buf[0].rgb;
    
    // Increase base brightness significantly
    color = color * 3.0 + 0.15;
    
    // Apply purple tinting
    color.r = color.r * 0.9 + 0.15;  // Add purple/red tint
    color.g = color.g * 0.6 + 0.05;  // Reduce green for purple
    color.b = color.b * 1.0 + 0.2;   // Add blue for purple
    
    // Add silver highlights for bright areas
    float brightness = dot(color, vec3(0.299, 0.587, 0.114));
    if (brightness > 0.6) {
      color = mix(color, vec3(0.85, 0.85, 0.9), 0.2); // Silver tint
    }
    ...
    <div ref={canvasRef} className="absolute inset-0 -z-10 w-full h-full" aria-hidden>
      <Canvas
        ...
        gl={{ 
          ...
          // Fix for StrictMode double mounting
          failIfMajorPerformanceCaveat: false,
          powerPreference: 'default'
        }}
        dpr={[1, 2]}
        style={{ width: '100%', height: '100%' }}
        // Add key to force remount on dev hot reload
        key="neural-canvas"
      >
        <ShaderPlane />
      </Canvas>
