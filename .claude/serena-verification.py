...
def check_python_environment() -> bool:
    ...
    try:
        code = "import fastapi, sqlalchemy, pydantic; print('imports ok')"
...
def check_mcp_server() -> None:
    ...
    logger.info("\n📊 MCP Server Status:")
    serena_status = (
        "✅ Active" if Path(".claude/serena-active.flag").exists() else "⚠️  Not initialized"
    )
...
def main() -> int:
    ...
    logger.info(
        "[%s] 🔍 Starting Serena MCP verification", datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    )
    ...
    if all_passed:
        logger.info(
            "[%s] ✅ MCP verification complete", datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
