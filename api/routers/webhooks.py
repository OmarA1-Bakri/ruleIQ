...
router = APIRouter(
    prefix="/api/v1/webhooks", tags=["Webhooks"], responses={(404): {"description": "Not found"}}
)


async def handle_stripe_event(event_type: str, data: dict, db: AsyncSession) -> None:
    ...
    if event_type == "payment_intent.succeeded":
        ...
        logger.info("Payment succeeded for customer %s: $%s" % (customer_id, amount))
    elif event_type == "customer.subscription.created":
        subscription = data.get("object", {})
        customer_id = subscription.get("customer")
        status = subscription.get("status")
        logger.info("Subscription created for customer %s: %s" % (customer_id, status))
...
async def handle_github_event(event_type: str, data: dict, db: AsyncSession) -> None:
...
async def handle_sendgrid_event(events: list, db: AsyncSession) -> None:
    ...
    for event in events:
        ...
        logger.info("Processing SendGrid event: %s for %s" % (event_type, email))
...
async def stripe_webhook(
    ...
    try:
        ...
        is_valid, error = await service.verify_webhook(
            request=request, provider="stripe", payload=payload
        )
        if not is_valid:
        ...
        background_tasks.add_task(handle_stripe_event, event_type, event_data, db)
...
async def github_webhook(
    ...
    try:
        ...
        service = WebhookVerificationService(redis_client)
        is_valid, error = await service.verify_webhook(
            request=request, provider="github", payload=payload
        )
        if not is_valid:
...
async def sendgrid_webhook(
    ...
    try:
        ...
        service = WebhookVerificationService(redis_client)
        is_valid, error = await service.verify_webhook(
            request=request, provider="sendgrid", payload=payload
        )
        if not is_valid:
...
async def custom_webhook(
    ...
    try:
        ...
        service = WebhookVerificationService(redis_client)
        is_valid, error = await service.verify_webhook(
            request=request, provider="custom", secret=webhook_secret, payload=payload
        )
        if not is_valid:
            logger.warning("Invalid custom webhook signature for %s: %s" % (webhook_id, error))
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid webhook signature"
            )
        try:
...
async def test_generate_signature(
    payload: str = '{"test": "data"}', secret: str = "test_secret", provider: str = "custom"
) -> Dict[str, Any]:
    """
    ...
    signature = await service.generate_webhook_signature(
        payload=payload, secret=secret, provider=provider
    )
    return {
        "provider": provider,
        "signature": signature,
