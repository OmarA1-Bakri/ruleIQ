...
async def get_plan(
    ...
    if not plan:
        raise HTTPException(status_code=HTTP_NOT_FOUND, detail="Implementation plan not found")
    total_phases = len(plan.phases) if plan.phases else 0
    total_tasks = sum(len(phase.get("tasks", [])) for phase in plan.phases) if plan.phases else 0
    all_tasks = []
        ...
        "recommendations": [
            {
                ...
                "description": "Enable comprehensive audit logging across all systems",
            ...
            "templates": [
                {
                    ...
                    "format": "docx",
                    "size_kb": 245,
                    "download_url": f"/api/v1/resources/{frameworkId}/templates/risk-assessment.docx",
                },
                {
                    "id": "tpl_002",
                    "name": "Policy Templates Bundle",
                    "type": "bundle",
                    "format": "zip",
                    "size_kb": 1024,
                    "download_url": f"/api/v1/resources/{frameworkId}/templates/policies-bundle.zip",
                },
            ],
            "guides": [
                {
                    "id": "guide_001",
                    "title": f"{frameworkId} Implementation Guide",
                    "description": "Step-by-step guide for implementing {frameworkId}",
                    "pages": 45,
                    "format": "pdf",
                    "url": f"/api/v1/resources/{frameworkId}/guides/implementation-guide.pdf",
                },
                {
                    "id": "guide_002",
                    "title": "Quick Start Checklist",
                    "description": "Essential checklist for getting started",
                    "pages": 5,
                    "format": "pdf",
                    "url": f"/api/v1/resources/{frameworkId}/guides/quick-start.pdf",
                },
            ],
            "tools": [
                {
                    "id": "tool_001",
                    "name": "Gap Analysis Spreadsheet",
                    "description": "Excel tool for conducting gap analysis",
                    "type": "spreadsheet",
                    "download_url": f"/api/v1/resources/{frameworkId}/tools/gap-analysis.xlsx",
                }
            ],
            "videos": [
                {
                    "id": "vid_001",
                    "title": f"Understanding {frameworkId}",
                    "duration_minutes": 15,
                    "url": f"https://videos.example.com/{frameworkId}-intro",
                    "thumbnail": f"https://thumbnails.example.com/{frameworkId}-intro.jpg",
                }
            ],
        },
        "total_resources": 7,
...
async def get_implementation_plan_analytics(
    ...
    return {
        ...
        "analytics": {
            ...
            "timeline": {
                "planned_start": (datetime.now(timezone.utc) - timedelta(days=30)).isoformat(),
                "planned_end": (datetime.now(timezone.utc) + timedelta(days=60)).isoformat(),
                "actual_start": (datetime.now(timezone.utc) - timedelta(days=30)).isoformat(),
                "projected_end": (datetime.now(timezone.utc) + timedelta(days=65)).isoformat(),
            ...
            "recommendations": [
                ...
                "Schedule checkpoint review for next week",
            ],
            "quality_metrics": {
                "tasks_passed_review": 22,
