...
def create_ai_rate_limit_dependency(limiter: AIRateLimiter, operation_name: str) -> Any:
    ...
    async def ai_rate_limit_check(
        ...
        if not allowed:
            ...
            error_detail = {
                "error": {
                    "message": f"AI {operation_name} rate limit exceeded",
                    "code": "AI_RATE_LIMIT_EXCEEDED",
                    "operation": operation_name,
                    "limit": limiter.requests_per_minute,
                    "window": "1 minute",
                    "retry_after": retry_after,
                    "burst_allowance": limiter.burst_allowance,
                },
                "suggestion": f"Please wait {retry_after} seconds before making another {operation_name} request.",
            }
            raise HTTPException(
...
ai_analysis_rate_limit = create_ai_rate_limit_dependency(ai_analysis_limiter, "analysis")
...
class AIRateLimitStats:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "requests_by_operation": dict(self.requests_by_operation),
            "rate_limits_by_operation": dict(self.rate_limits_by_operation),
