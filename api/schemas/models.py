...
class RecommendationDetail(BaseModel):
    ...
    estimated_effort: str = Field(
        ..., pattern="^(Low|Medium|High)$", description="Estimated effort to implement."
    )
...
class BusinessProfileCreate(BaseModel):
    ...
    provides_financial_services: bool = Field(
        default=False, description="Provides financial services"
    )
    operates_critical_infrastructure: bool = Field(
        default=False, description="Operates critical infrastructure"
    )
    has_international_operations: bool = Field(
        default=False, description="Has international operations"
    )
    existing_frameworks: Optional[List[str]] = Field(default_factory=list)
    planned_frameworks: Optional[List[str]] = Field(default_factory=list)
    cloud_providers: Optional[List[str]] = Field(default_factory=list)
...
class PolicyGenerateRequest(BaseModel):
    ...
    policy_type: Optional[str] = Field(
        "comprehensive", description="The type of policy to generate."
    )
    custom_requirements: Optional[List[str]] = Field(
        None, description="A list of custom requirements to include."
    )
...
class AssessmentQuestion(BaseModel):
    ...
    question_type: str = Field(
        ..., pattern="^(multiple_choice|free_text|yes_no)$", description="The type of question."
    )
...
class ComplianceReport(BaseModel):
    ...
    framework: Optional[str] = Field(
        None, description="The name or ID of the compliance framework for the report."
    )
    ...
    format: str = Field(
        default="pdf", pattern="^(pdf|json|docx)$", description="The output format for the report."
    )
