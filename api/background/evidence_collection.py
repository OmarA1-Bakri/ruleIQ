...
async def execute_foundation_evidence_collection(
    ...
    async for db in get_async_db():
        try:
            ...
            logger.info("Starting evidence collection %s for user %s" % (collection_id, user_id))
            total_evidence_types = len(evidence_types)
            completed_types = []
            for provider, integration in integration_map.items():
                try:
                    logger.info("Starting evidence collection from %s" % provider)
                    ...
                    if not provider_evidence_types:
                        logger.info("No relevant evidence types for %s" % provider)
                        continue
                    for evidence_type in provider_evidence_types:
                        try:
                            ...
                            logger.info(
                                "Completed %s collection from %s" % (evidence_type, provider)
                            )
                        except Exception as e:
                            logger.error(
                                "Failed to collect %s from %s: %s" % (evidence_type, provider, e)
                            )
                            continue
                    await client.close()
                except Exception as e:
                    logger.error("Failed to collect from %s: %s" % (provider, e))
                    continue
            await evidence_service.update_collection_status(
                collection_id=collection_id,
                status="completed",
            ...
            logger.info("Evidence collection %s completed successfully" % collection_id)
        except Exception as e:
            logger.error("Evidence collection %s failed: %s" % (collection_id, e))
            try:
                await evidence_service.update_collection_status(
                    collection_id=collection_id,
                    status="failed",
                ...
                logger.error("Failed to update collection status: %s" % update_error)
        finally:
            await db.close()
            break
