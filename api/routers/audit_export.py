...
async def get_recent_audit_logs(
    ...
    query = (
        select(AuditLog)
        .where(and_(AuditLog.user_id == current_user.id, AuditLog.timestamp >= cutoff_date))
    ...
    query = (
        ...
        .where(
            and_(
                ...
                or_(*[AuditLog.action.contains(action) for action in security_actions]),
            )
        )
        .order_by(AuditLog.timestamp.desc())
    )
        ...
        events_by_type[event_type].append(
            {
                ...
                "ip_address": log.ip_address,
                "status": log.status,
                "details": json.loads(log.metadata) if log.metadata else {},
            }
        )
    failed_logins = len([log for log in logs if "login_failure" in log.action])
    successful_logins = len([log for log in logs if "login_success" in log.action])
    return {
        "period_days": days,
        ...
        "recommendation": _get_security_recommendation(failed_logins, successful_logins),
    }
...
async def generate_compliance_report(
    ...
    query = select(AuditLog).where(
        and_(AuditLog.user_id == current_user.id, AuditLog.timestamp >= cutoff_date)
    )
    result = await db.execute(query)
    logs = result.scalars().all()
    ...
    report = {
        ...
        "summary": {
            ...
            "data_modifications": len(modification_logs),
            "security_events": len(security_logs),
        },
        "compliance_checks": {
            "audit_trail_maintained": len(logs) > 0,
            "user_activity_tracked": True,
            "data_access_logged": len(data_access_logs) > 0,
            "modifications_tracked": len(modification_logs) > 0,
            "security_events_monitored": len(security_logs) > 0,
        },
        "recommendations": _get_compliance_recommendations(logs),
    }
    if framework:
        report["framework_specific"] = _get_framework_specific_report(framework, logs)
    return report
...
def _export_json(logs: List[AuditLog], user: User) -> Response:
    ...
    filename = f"audit_log_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    return Response(
...
def _export_txt(logs: List[AuditLog], user: User) -> StreamingResponse:
    ...
    output = io.StringIO()
    output.write(f"Audit Log Export - {datetime.now(timezone.utc).isoformat()}\n")
    ...
    for log in logs:
        ...
        output.write("\n")
    output.seek(0)
    filename = f"audit_log_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.txt"
    return StreamingResponse(
        io.BytesIO(output.getvalue().encode()),
...
def _get_security_recommendation(failed_logins: int, successful_logins: int) -> str:
    """Generate security recommendation based on events."""
    if failed_logins > successful_logins * 0.5:
        return "High number of failed login attempts detected. Consider enabling MFA."
    elif failed_logins > 10:
        return "Multiple failed login attempts detected. Review account security."
    else:
...
def _is_security_event(action: str) -> bool:
    ...
    security_keywords = ["login", "logout", "password", "permission", "role", "mfa", "auth"]
...
def _get_compliance_recommendations(logs: List[AuditLog]) -> List[str]:
    ...
    if not backup_logs:
        recommendations.append("No backup activity detected - implement regular data backups")
    review_logs = [
        log for log in logs if "review" in log.action.lower() or "audit" in log.action.lower()
    ]
    if not review_logs:
...
def _get_framework_specific_report(framework: str, logs: List[AuditLog]) -> Dict:
    """Generate framework-specific compliance report section."""
    if framework.upper() == "GDPR":
        return {
            "data_access_requests": len(
                [log for log in logs if "data_access_request" in log.action]
            ),
            "data_deletion_requests": len([log for log in logs if "data_deletion" in log.action]),
            "consent_updates": len([log for log in logs if "consent" in log.action]),
            "breach_notifications": len([log for log in logs if "breach" in log.action]),
        }
    elif framework.upper() == "ISO27001":
        return {
            "access_control_events": len([log for log in logs if _is_security_event(log.action)]),
            "change_management": len([log for log in logs if _is_modification(log.action)]),
            "incident_responses": len([log for log in logs if "incident" in log.action]),
            "risk_assessments": len([log for log in logs if "risk" in log.action]),
        }
    else:
