...
def get_secret_or_env(secret_key: str, env_key: Optional[str] = None) -> Optional[str]:
    ...
    value = os.getenv(env_key)
    if value:
        logger.debug("🔐 Retrieved '%s' from environment (%s)" % (secret_key, env_key))
        return value
    logger.warning("⚠️ Secret '%s' not found in vault or environment" % secret_key)
    return None
...
class Settings(BaseSettings):
    ...
    port: int = Field(
        default_factory=lambda: int(os.getenv("PORT", 8000)), description="Port to bind"
    )
    ...
    redis_url: str = Field(
        default_factory=lambda: (
            ...
            if os.getenv("TESTING", "").lower() == "true"
                ...
                SecretKeys.REDIS_URL if SECRETS_VAULT_AVAILABLE else "redis_url", "REDIS_URL"
            )
            or ""
        ),
    ...
    jwt_secret_key: str = Field(
        default_factory=lambda: (
            ...
            if os.getenv("TESTING", "").lower() == "true"
                ...
                SecretKeys.JWT_SECRET if SECRETS_VAULT_AVAILABLE else "jwt_secret", "JWT_SECRET_KEY"
            )
            or ""
        ),
    ...
    cache_migration_on_startup: bool = Field(
        default=False, description="Invalidate MD5-based caches on startup (one-time migration)"
    )
    ...
    celery_task_timeout: int = Field(
        default=300, description="DEPRECATED: Celery task timeout (seconds)"
    )

    @field_validator(
        "celery_broker_url", "celery_result_backend", "celery_task_timeout", mode="after"
    )
    @classmethod
    ...
    sentry_dsn: Optional[str] = Field(
        default_factory=lambda: (
            ...
            if os.getenv("TESTING", "").lower() == "true"
                ...
                SecretKeys.SENTRY_DSN if SECRETS_VAULT_AVAILABLE else "sentry_dsn", "SENTRY_DSN"
            )
        ),
    ...
    slow_request_threshold: float = Field(
        default=1.0, description="Slow request warning threshold (seconds)"
    )
    ...
    disk_warning_threshold: float = Field(
        default=80.0, description="Disk usage warning threshold (%)"
    )
    disk_critical_threshold: float = Field(
        default=90.0, description="Disk usage critical threshold (%)"
    )
    memory_warning_threshold: float = Field(
        default=85.0, description="Memory usage warning threshold (%)"
    )
    memory_critical_threshold: float = Field(
        default=95.0, description="Memory usage critical threshold (%)"
    )
    ...
    real_time_security_alerts: bool = Field(
        default=True, description="Enable real-time security alerts"
    )
        ...
        if not v or not v.startswith(("postgresql://", "postgresql+asyncpg://", "sqlite:///")):
            if os.getenv("TESTING", "").lower() == "true":
                ...
                if test_url:
                    return test_url
            raise ValueError(
                "Database URL must be provided and start with postgresql:// or sqlite:///"
            )
        return v
    ...
    def validate_jwt_secret(cls, v: str) -> str:
        ...
        if not v or len(v) < 32:
            raise ValueError(
                "JWT secret key must be at least 32 characters long and provided via environment"
            )
        return v
    ...
    def log_vault_status(self) -> None:
        ...
        logger.info(
            "🔐 Vault Configuration: enabled=%s, region=%s"
