...
class EnhancedTokenBlacklist:
    ...
    async def blacklist_token(self, token: str, reason: str='logout', ttl:
        Optional[int]=None, user_id: Optional[str]=None, session_id:
        Optional[str]=None, ip_address: Optional[str]=None, user_agent:
        Optional[str]=None, metadata: Optional[Dict]=None) ->bool:
        """
        ...
        try:
            ...
            entry = BlacklistEntry(token_hash=token_hash, reason=reason,
                blacklisted_at=datetime.now(timezone.utc), expires_at=
                datetime.now(timezone.utc) + timedelta(seconds=ttl),
                user_id=user_id, session_id=session_id, ip_address=
                ip_address, user_agent=user_agent, metadata=metadata)
            entry_data = {**asdict(entry), 'blacklisted_at': entry.
                blacklisted_at.isoformat(), 'expires_at': entry.expires_at.
                isoformat()}
            await self.cache_manager.set(cache_key, json.dumps(entry_data),
                ttl=ttl)
            await self._update_metrics('blacklisted')
            await self._analyze_blacklist_patterns(entry)
            logger.info(
                'Token blacklisted: hash=%s..., reason=%s, user_id=%s' % (
                token_hash[:8], reason, user_id))
            return True
    ...
    async def is_token_blacklisted(self, token: str) ->bool:
        ...
        try:
            ...
            logger.error('Failed to check token blacklist status: %s' % e)
            return False
    ...
    async def get_blacklist_entry(self, token: str) ->Optional[BlacklistEntry]:
        ...
        try:
            ...
            data['blacklisted_at'] = datetime.fromisoformat(data[
                'blacklisted_at'])
            data['expires_at'] = datetime.fromisoformat(data['expires_at'])
            return BlacklistEntry(**data)
        except Exception as e:
            logger.error('Failed to get blacklist entry: %s' % e)
            return None
    ...
    async def remove_token_from_blacklist(self, token: str) ->bool:
        ...
        try:
            ...
            if result:
                logger.info('Token removed from blacklist: hash=%s...' %
                    token_hash[:8])
            return result
        except Exception as e:
    ...
    async def blacklist_user_tokens(self, user_id: str, reason: str=
        'security_action', exclude_current_token: Optional[str]=None) ->int:
        """
        ...
        try:
            ...
            logger.error('Failed to blacklist user tokens: %s' % e)
            return 0
    ...
    async def cleanup_expired_tokens(self) ->int:
        ...
        try:
            ...
            logger.info('Cleaned up %s expired tokens' % cleaned_count)
    ...
    async def get_blacklist_statistics(self) ->Dict:
        ...
        try:
            pattern = f'{BLACKLIST_PREFIX}:*'
            current_count = len(await self.cache_manager.redis_client.keys(
                pattern))
            return {'current_blacklisted_tokens': current_count,
                'total_blacklisted': self.metrics.total_blacklisted,
                'blacklisted_today': self.metrics.blacklisted_today,
                'expired_tokens_cleaned': self.metrics.
                expired_tokens_cleaned, 'suspicious_patterns_detected':
                self.metrics.suspicious_patterns_detected,
                'bulk_operations_count': self.metrics.bulk_operations_count,
                'last_cleanup': self.metrics.last_cleanup.isoformat() if
                self.metrics.last_cleanup else None}
        except Exception as e:
            logger.error('Failed to get blacklist statistics: %s' % e)
            return {}
    ...
    async def _save_metrics(self) ->None:
        ...
        try:
            metrics_data = {**asdict(self.metrics), 'last_cleanup': self.
                metrics.last_cleanup.isoformat() if self.metrics.
                last_cleanup else None}
            await self.cache_manager.set(BLACKLIST_METRICS_KEY, json.dumps(
                metrics_data), ttl=86400)
        except Exception as e:
        ...
        try:
            ...
            if metrics_data:
                ...
                if data.get('last_cleanup'):
                    data['last_cleanup'] = datetime.fromisoformat(data[
                        'last_cleanup'])
    ...
    async def _analyze_blacklist_patterns(self, entry: BlacklistEntry) ->None:
        ...
        try:
            if entry.ip_address and entry.user_id:
                recent_pattern_key = (
                    f'pattern:{entry.ip_address}:{entry.user_id}')
