...
class CostTrackingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self,
        app,
        cost_manager: Optional[AICostManager] = None,
    ...
    async def dispatch(
        ...
        try:
            ...
            logger.error("Unexpected error in cost tracking middleware: %s" % str(e))
            return JSONResponse(
    ...
    def _extract_service_name(self, request: Request) -> str:
        ...
        service_mapping = {
            ...
            "/api/v1/ai/refine-policy": "policy_refinement",
            "/api/v1/ai/assessments": "assessment_analysis",
            "/api/v1/ai/chat": "chat_assistance",
    ...
    async def _check_user_budget_limits(self, user_id: str) -> None:
        ...
        try:
            ...
            logger.debug("Checking budget limits for user: %s, key: %s" % (user_id, user_limit_key))
            ...
            if today_usage:
                ...
                if total_cost >= daily_limit:
                    ...
                    logger.warning(
                        "User %s approaching daily cost limit: $%s" % (user_id, total_cost)
                    )
        ...
        try:
            ...
            if token_usage:
                result = await self.cost_manager.track_ai_request(
                    ...
                    input_prompt=token_usage.get("input_prompt", ""),
                    response_content=token_usage.get("response_content", ""),
                    ...
                    cache_hit=token_usage.get("cache_hit", False),
                    error_occurred=False,
                    metadata={
                        "endpoint": tracking_info["endpoint"],
                ...
                logger.debug(
                    "Tracked successful request %s: $%s"
    ...
    async def _track_failed_request(
        self, request: Request, error: HTTPException, start_time: float
    ) -> None:
            ...
            if error.status_code >= HTTP_INTERNAL_SERVER_ERROR:
                ...
                if estimated_tokens:
                    await self.cost_manager.track_ai_request(
                        service_name=tracking_info["service_name"],
                        model_name=estimated_tokens.get("model_name", "unknown"),
                        input_prompt="",
                        response_content="",
    ...
    def _mock_token_usage(self, request: Request) -> Dict[str, Any]:
        ...
        if "generate-policy" in path:
            return {
                ...
                "input_tokens": 1500,
                "output_tokens": 2000,
                "input_prompt": "Generate privacy policy...",
            ...
            return {
                ...
                "input_tokens": 1200,
                "output_tokens": 1500,
                "input_prompt": "Assessment analysis...",
    ...
    def _estimate_failed_request_tokens(self, request: Request) -> Optional[Dict[str, Any]]:
        ...
        try:
            return {"model_name": "gemini-1.5-flash", "input_tokens": 500, "output_tokens": 0}
    ...
    def _add_cost_headers(self, response: Response, request: Request) -> None:
        ...
        try:
            if hasattr(request.state, "cost_result"):
                ...
                if hasattr(request.state, "cost_tracking") and request.state.cost_tracking.get(
                    "user_id"
                ):
...
class RealTimeCostMonitor:
    ...
    async def connect_client(self, client_id: str, websocket) -> None:
        ...
        logger.info("Client %s connected for real-time cost monitoring" % client_id)

    async def disconnect_client(self, client_id: str) -> None:
        """Disconnect client from real-time updates."""
        if client_id in self.active_connections:
            del self.active_connections[client_id]
            logger.info("Client %s disconnected from cost monitoring" % client_id)
    ...
    async def broadcast_cost_update(self, cost_data: Dict[str, Any]) -> None:
        ...
        for client_id, connection in self.active_connections.items():
            try:
                ...
                logger.warning("Failed to send cost update to client %s: %s" % (client_id, str(e)))
    ...
    async def send_budget_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        for client_id, connection in self.active_connections.items():
            try:
                ...
                logger.warning("Failed to send budget alert to client %s: %s" % (client_id, str(e)))
