...
async def configure_aws_integration(
    ...
    try:
        ...
        if not await aws_client.authenticate():
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="AWS authentication failed")
        ...
        logger.error(
            "Error configuring AWS integration for user %s: %s" % (str(current_user.id), e)
        )
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}"
        )
...
async def configure_okta_integration(
    ...
    try:
        ...
        if not await okta_client.authenticate():
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="Okta authentication failed")
        ...
        logger.error(
            "Error configuring Okta integration for user %s: %s" % (str(current_user.id), e)
        )
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}"
        )
...
async def start_foundation_evidence_collection(
    ...
    try:
        ...
        logger.error(
            "Error starting evidence collection for user %s: %s" % (str(current_user.id), e)
        )
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to start collection: {str(e)}"
        )
...
async def get_collection_status(
    ...
    try:
        ...
        if not collection:
            raise HTTPException(status_code=HTTP_NOT_FOUND, detail="Collection not found")
        total_expected = (
        ...
        logger.error("Error getting collection status %s: %s" % (collection_id, e))
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to get status: {str(e)}"
        )
...
async def get_collection_results(
    ...
    try:
        ...
        logger.error("Error getting collection results %s: %s" % (collection_id, e))
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to get results: {str(e)}"
        )
...
async def check_foundation_integrations_health(
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Health check failed: {str(e)}"
        )
