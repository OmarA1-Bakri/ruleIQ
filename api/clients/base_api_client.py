...
class BaseAPIClient(ABC):
    ...
    async def ensure_authenticated(self) -> bool:
        ...
        if (
            ...
            or (now - self.last_auth_check).total_seconds() > HOUR_IN_SECONDS
        ):
        ...
        for attempt in range(request.retry_attempts):
            try:
                ...
                if attempt < request.retry_attempts - 1:
                    ...
                    logger.warning(
                        "Rate limited by %s, retrying in %ss" % (self.provider_name, wait_time)
                    )
    ...
    async def collect_evidence(self, evidence_type: str, **kwargs: Any) -> List[EvidenceItem]:
