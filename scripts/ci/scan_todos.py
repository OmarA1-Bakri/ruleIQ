...
def should_ignore_todo(item: TodoItem) -> bool:
    ...
    path_lower = str(item.file_path).lower()
    return "example" in path_lower or "template" in path_lower
...
def analyze_todos(todos: List[TodoItem]) -> Dict[str, Any]:
    ...
    return {
        ...
        "compliance_rate": (sum(1 for t in todos if t.issue_number) / len(todos) * 100)
        if todos
        else 100.0,
    }
...
def generate_markdown_report(todos: List[TodoItem], stats: Dict[str, Any]) -> str:
    ...
    report.append(
        f"- With Issue References: {stats['with_issues']} ({stats['compliance_rate']:.1f}%)"
    )
    report.append(
        f"- Without Issue References: {stats['without_issues']} ({100 - stats['compliance_rate']:.1f}%)"
    )
    ...
    for severity in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
        ...
        for i, todo in enumerate(severity_todos[:50], 1):  # Limit to 50 per severity
            ...
            report.append(
                f"{i}. {compliance} `{todo.file_path}:{todo.line_number}` - {todo.marker}{issue_ref}: {todo.description[:80]}"
            )
    ...
    if stats["without_issues"] > 0:
        report.append(
            f"3. 📝 Update {stats['without_issues']} non-compliant TODOs to reference issues"
        )
...
def main():
    ...
    parser.add_argument(
        "--format", choices=["markdown", "json", "csv"], default="markdown", help="Output format"
    )
    parser.add_argument("--output", type=Path, help="Output file path")
    parser.add_argument("--enforce", action="store_true", help="Fail if non-compliant TODOs found")
    parser.add_argument(
        "--severity",
        action="append",
    ...
    if args.enforce:
        non_compliant = [
            t for t in todos if not t.issue_number and t.severity in ["CRITICAL", "HIGH"]
        ]
        if non_compliant:
            ...
            for todo in non_compliant[:10]:
                print(
                    f"  {todo.file_path}:{todo.line_number} - {todo.marker}: {todo.description[:60]}"
                )
