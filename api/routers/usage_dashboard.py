...
async def get_usage_dashboard(
    ...
    for feature, _limits in RateLimitService.LIMITS.items():
        usage_info = await RateLimitService.check_rate_limit(
            db, current_user, feature, check_only=True
        )
        stats = UsageStats(
            feature=feature.replace("_", " ").title(),
            used_today=usage_info["used_today"],
    ...
    stmt = (
        ...
        .where(
            and_(
                ...
                AuditLog.action.like("rate_limit:%"),
    ...
    for log in audit_logs:
        try:
            ...
            history = UsageHistory(
                ...
                feature=log.action.replace("rate_limit:", "").replace("_", " ").title(),
                action=metadata.get("action", "usage"),
                metadata=metadata,
            )
    ...
    most_used_features = [
        ...
        for k, v in sorted(feature_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ]
...
async def get_usage_limits(current_user: User = Depends(get_current_active_user)) -> Any:
    ...
    for feature, config in RateLimitService.LIMITS.items():
        limits[feature.replace("_", " ").title()] = {
...
async def get_feature_usage(
    ...
    if feature_key not in RateLimitService.LIMITS:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Feature '{feature}' not found"
        )
    usage_info = await RateLimitService.check_rate_limit(
        db, current_user, feature_key, check_only=True
    )
    return UsageStats(
        feature=feature.replace("_", " ").title(),
        used_today=usage_info["used_today"],
...
def _get_feature_description(feature: str) -> str:
    ...
    descriptions = {
        "ai_assessment": "AI-powered assessment help and question assistance",
        "ai_policy_generation": "Generate compliance policies using AI",
        "ai_compliance_check": "Validate policies against compliance frameworks",
        "ai_recommendation": "Get personalized compliance recommendations",
    }
