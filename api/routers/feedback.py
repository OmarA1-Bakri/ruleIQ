...
async def submit_feedback(
    ...
    try:
        ...
        if success:
            logger.info("Feedback submitted successfully: %s" % feedback_item.feedback_id)
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to store feedback"
            )
    ...
    for feedback_request in request.feedback_items:
        try:
            ...
            if success:
                ...
                responses.append(
                    FeedbackResponse(
                        feedback_id="", status="failed", message="Failed to store feedback"
                    )
                )
        except Exception as e:
            logger.error("Error in batch submission: %s" % str(e))
            responses.append(FeedbackResponse(feedback_id="", status="error", message=str(e)))
    return responses
...
async def retrieve_feedback_by_id(
    ...
    try:
        ...
        if not feedback:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Feedback {feedback_id} not found"
            )
        if feedback.get("user_id") != str(current_user.id) and not current_user.is_admin:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to view this feedback"
            )
        return feedback
...
async def export_feedback(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Admin permission required for export"
        )
    try:
