    ...
    try:
        ...
        if business_user_role:
            rbac_service.assign_role_to_user(
                user_id=db_user.id, role_id=business_user_role.id, granted_by=db_user.id
            )
...
async def login_for_access_token(
    ...
    try:
        ...
        form_data.username = SecurityValidator.validate_no_dangerous_content(
    ...
    if not user:
        logger.warning(
            "Login attempt for non-existent user: %s from IP: %s" % (form_data.username, ip_address)
        )
        raise HTTPException(
...
async def login(
    ...
    if not user:
        logger.warning(
            "Login attempt for non-existent user: %s from IP: %s" % (login_data.email, ip_address)
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def assign_default_role(
    ...
    try:
        ...
        rbac.assign_role_to_user(user_id=user.id, role_id=business_user_role.id, granted_by=user.id)
