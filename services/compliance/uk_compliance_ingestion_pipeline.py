...
class UKComplianceGraphIngestion:
    ...
    def __init__(self, neo4j_uri: str, neo4j_auth: tuple, embedding_model=None) -> None:
    ...
    async def _ingest_document(self, doc_data: Dict, source: str):
        ...
        async with self.driver.session() as session:
            doc_id = doc_data.get(
                "id", hashlib.md5(json.dumps(doc_data).encode(), usedforsecurity=False).hexdigest()
            )
            await session.run(
                """
                MERGE (d:Document {id: $doc_id})
...
class UKComplianceGraphQuery:
    ...
    async def find_similar_obligations(self, obligation_id: str, limit: int = 5) -> List[Dict]:
        ...
        async with self.driver.session() as session:
            ...
            async for record in result:
                records.append(
                    {"obligation": dict(record["o2"]), "similarity": record["similarity"]}
                )
            if not records:
                result = await session.run(
                    """
