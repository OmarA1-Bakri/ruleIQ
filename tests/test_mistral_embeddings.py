...
async def test_mistral_embeddings():
    ...
    try:
        ...
        print(f"   Mistral Client: {'Available' if rag_system.mistral_client else 'Not available'}")
        print(f"   OpenAI Client: {'Available' if rag_system.openai_client else 'Not available'}")
        if rag_system.use_mistral_embeddings and rag_system.mistral_client:
            print("\n🔄 Testing Mistral embedding generation...")
            test_text = "LangGraph provides state management for complex AI workflows"
            ...
            print("   Expected: 1536 dimensions (OpenAI text-embedding-3-small)")
        else:
