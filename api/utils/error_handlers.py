...
class ErrorHandler:
    ...
    def create_error_response(
        ...
        if isinstance(exception, RuleIQException):
            ...
            logger.error(
                "Unexpected error: %s" % str(exception),
                extra={
                    "error": str(exception),
    ...
    def log_error(
        ...
        logger.error("Application error: %s" % str(exception), extra=log_data, exc_info=True)
...
async def global_exception_handler(request: Request, exception: Exception) -> JSONResponse:
    ...
    if "details" in error_response["error"]:
        error_response["error"]["details"] = ErrorHandler.sanitize_error_details(
...
class ValidationUtils:
    ...
    def validate_required_fields(data: Dict[str, Any], required_fields: List[str]) -> None:
        ...
        if missing_fields:
            raise ValidationException(
                "Missing required fields", details={"missing_fields": missing_fields}
            )
...
class SecurityErrorHandler:
    ...
    def handle_authentication_error(details: Optional[Dict[str, Any]] = None) -> NoReturn:
        ...
        logger.warning(
            ...
            extra={"details": ErrorHandler.sanitize_error_details(details or {})},
        )
    ...
    def handle_authorization_error(
        ...
        logger.warning(
            ...
            extra={"resource": resource, "action": action, "user_id": user_id},
        )
...
def create_error_template(
    ...
    return RuleIQException(
        message=message, error_code=code, status_code=status_code, details=details
    )
