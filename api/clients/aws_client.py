...
class AWSAPIClient(BaseAPIClient):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "AWS authentication successful for account: %s" % caller_identity.get("Account")
            )
            return True
...
class AWSIAMEvidenceCollector(BaseEvidenceCollector):
            ...
            for page in policies_paginator.paginate(Scope="Local"):
                for policy in page["Policies"]:
                    try:
                        ...
                        self.logger.warning(
                            "Failed to get details for policy %s: %s" % (policy["PolicyName"], e)
                        )
                        continue
        except ClientError as e:
            ...
            for page in users_paginator.paginate():
                for user in page["Users"]:
                    try:
                        ...
                        self.logger.warning(
                            "Failed to get details for user %s: %s" % (user["UserName"], e)
                        )
                        continue
        except ClientError as e:
            ...
            for page in roles_paginator.paginate():
                for role in page["Roles"]:
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "max_session_duration": role.get(
                                    "MaxSessionDuration", DEFAULT_SESSION_DURATION
                                ),
                        ...
                        self.logger.warning(
                            "Failed to get details for role %s: %s" % (role["RoleName"], e)
                        )
                        continue
        except ClientError as e:
    ...
    def _calculate_policy_quality_score(
        self, policy: Dict[str, Any], policy_document: Any
    ) -> float:
        ...
        try:
            ...
            for lookup_attr in lookup_attributes[:5]:
                try:
                    ...
                    self.logger.warning(
                        "Failed to collect events for %s: %s" % (lookup_attr["AttributeValue"], e)
                    )
                    continue
        except ClientError as e:
