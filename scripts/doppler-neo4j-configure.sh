...
# Load credentials from the local environment instead of hard-coding them in-repo.
: "${NEO4J_URI:?Set NEO4J_URI before running this script}"
: "${NEO4J_USERNAME:?Set NEO4J_USERNAME before running this script}"
...
doppler secrets set NEO4J_INSTANCE_ID="${NEO4J_INSTANCE_ID:-set-in-environment}" --silent
doppler secrets set NEO4J_QUERY_API_URL="${NEO4J_QUERY_API_URL:-https://your-instance-id.databases.neo4j.io/db/neo4j/query/v2}" --silent
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "⚠️  Security Note: this script reads credentials from your local environment."
    echo "   Do not commit exported secret values or shell history containing them."
else
    echo ""
