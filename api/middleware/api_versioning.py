...
class APIVersioning:
    ...
    def mark_deprecated(self, endpoint: str, version: str, replacement: Optional[str]=None) -> None:
        ...
        self.deprecated_endpoints[endpoint] = {'version': version, 'replacement': replacement, 'deprecated_at': '2024-01-01'}

    def get_version_from_request(self, request: Request) -> str:
        """Extract API version from request."""
        path = request.url.path
        version_match = re.match('^/api/(v\\d+)/', path)
        if version_match:
    ...
    def get_version_info(self, version: str) -> Dict[str, Any]:
        ...
        return {'version': version, 'supported': version in self.SUPPORTED_VERSIONS, 'latest': version == self.LATEST_VERSION, 'deprecated': version != self.LATEST_VERSION}

    def create_version_response(self, version: str, data: Any) -> Dict[str, Any]:
        """Create response with version information."""
        response = {'data': data, 'meta': {'api_version': version, 'supported_versions': self.SUPPORTED_VERSIONS, 'latest_version': self.LATEST_VERSION}}
        if version != self.LATEST_VERSION:
            response['meta']['warning'] = f'API version {version} is deprecated. Use {self.LATEST_VERSION} instead.'
        return response
    ...
    def check_deprecation_warning(self, endpoint: str, version: str) -> Optional[Dict[str, str]]:
        ...
        if endpoint in self.deprecated_endpoints:
            ...
            return {'warning': f"Endpoint {endpoint} is deprecated since {deprecation['version']}", 'replacement': deprecation.get('replacement', 'Use latest version')}
        return None
...
class VersionMiddleware:
    ...
    async def __call__(self, request: Request, call_next) -> Any:
        ...
        if not self.versioning.validate_version(version):
            return JSONResponse(status_code=400, content={'error': {'code': 'INVALID_API_VERSION', 'message': f'Unsupported API version: {version}', 'supported_versions': self.versioning.SUPPORTED_VERSIONS}})
...
api_versioning.register_version('v2', {'description': 'Enhanced API with improved performance', 'status': 'latest'})
...
def version_route(version: str) -> Any:
...
VERSION_RESPONSE_SCHEMAS = {'v1': {'evidence': {'id': 'uuid', 'name': 'string', 'status': 'string', 'created_at': 'datetime'}, 'assessment': {'id': 'uuid', 'title': 'string', 'status': 'string', 'progress': 'integer'}}, 'v2': {'evidence': {'id': 'uuid', 'name': 'string', 'status': 'string', 'created_at': 'datetime', 'metadata': 'object', 'tags': 'array'}, 'assessment': {'id': 'uuid', 'title': 'string', 'status': 'string', 'progress': 'integer', 'ai_insights': 'object', 'recommendations': 'array'}}}
