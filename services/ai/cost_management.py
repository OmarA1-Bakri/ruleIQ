...
class ModelCostConfig:
    ...
    def calculate_output_cost(self, tokens: int) -> Decimal:
        ...
        return Decimal(tokens) / Decimal(1000000) * self.output_cost_per_million
...
class AIUsageMetrics:
    ...
    def aggregate(self, other: "AIUsageMetrics") -> "AIUsageMetrics":
        ...
        if self.service_name != other.service_name or self.model_name != other.model_name:
            raise ValueError("Cannot aggregate metrics from different services or models")
...
class CostTrackingService:
    ...
    def _load_model_configs(self) -> Dict[str, ModelCostConfig]:
        ...
        configs = {}
        gemini_models = ["gemini-1.5-pro", "gemini-1.5-flash", "gemini-2.5-pro", "gemini-2.5-flash"]
    ...
    async def track_usage(
        ...
        cost_usd = model_config.calculate_total_cost(input_tokens, output_tokens)
        ...
        logger.debug("Tracked usage: %s/%s - $%s" % (service_name, model_name, cost_usd))
        return usage
    ...
    async def _store_usage_in_redis(self, usage: AIUsageMetrics) -> None:
        ...
        service_key = f"ai_usage:service:{usage.service_name}:{timestamp.date()}"
        model_key = f"ai_usage:model:{usage.model_name}:{timestamp.date()}"
        user_key = f"ai_usage:user:{usage.user_id}:{timestamp.date()}" if usage.user_id else None
        usage_data = {
    ...
    async def _flush_usage_buffer(self) -> None:
        ...
        if not self.usage_buffer:
            return
        logger.info("Flushing %s usage records to persistent storage" % len(self.usage_buffer))
    ...
    async def calculate_daily_costs(self, target_date: date) -> Dict[str, Any]:
        ...
        service_breakdown = {}
        service_keys = await self.redis.keys(f"ai_usage:service:*:{target_date}")
        ...
        return {
            "total_cost": Decimal(daily_data.get("total_cost", "0")),
...
class BudgetAlertService:
    ...
    async def set_service_budget(self, service_name: str, amount: Decimal) -> None:
...
class CostOptimizationService:
    ...
    async def analyze_model_efficiency(self, usage_data: List[AIUsageMetrics]) -> CostOptimization:
        ...
        for model_name, usages in model_performance.items():
            ...
            efficiency = avg_quality / float(total_cost) if total_cost > 0 else 0
        ...
        best_model = max(model_efficiency.items(), key=lambda x: x[1]["efficiency"])
        current_cost = sum(u.cost_usd for u in usage_data)
        best_model_avg_cost = best_model[1]["cost"] / best_model[1]["usage_count"]
        estimated_savings = current_cost - len(usage_data) * best_model_avg_cost
        return CostOptimization(
            strategy=OptimizationStrategy.MODEL_SWITCH,
            recommendation=f"Switch to {best_model[0]} for better cost efficiency",
    ...
    async def analyze_caching_opportunities(
        ...
        for request_group in similar_requests:
            if request_group["count"] > 1:
                savings_per_cache = request_group["cost"] * (request_group["count"] - 1)
                total_savings += savings_per_cache
        return CostOptimization(
            strategy=OptimizationStrategy.CACHING_IMPROVEMENT,
            recommendation="Implement intelligent caching for repeated similar requests",
    ...
    async def analyze_batch_opportunities(
        ...
        return CostOptimization(
            ...
            recommendation=f"Batch {len(grouped_requests)} groups of requests for cost efficiency",
            potential_savings=total_savings,
    ...
    async def analyze_prompt_efficiency(self, prompt_metrics: Dict[str, Any]) -> CostOptimization:
        ...
        if avg_input_tokens > 2000:
            recommendations.append("Reduce input token count through prompt compression")
    ...
    async def generate_optimization_report(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "roi_analysis": {
                ...
                "projected_monthly_savings": total_potential_savings * 30,
...
class AICostManager:
    ...
    async def track_ai_request(
        ...
        usage = await self.cost_tracker.track_usage(
            service_name=service_name,
            model_name=model_name,
            input_tokens=input_tokens,
            output_tokens=output_tokens,
    ...
    async def get_daily_summary(self, target_date: date) -> Dict[str, Any]:
        ...
        daily_costs = await self.cost_tracker.calculate_daily_costs(
            target_date,
        )
    ...
    async def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if not usage_data:
            return []
        model_optimization = await self.optimization_service.analyze_model_efficiency(usage_data)
        return [
            {
                "strategy": model_optimization.strategy.value,
...
class CostAnalyticsDashboard:
    ...
    async def generate_executive_summary(self, cost_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "roi_analysis": {"cost_per_customer": current_month / 100, "efficiency_score": 85.5},
            "optimization_impact": {
        ...
        if len(costs) < 2:
            ...
            for day in range(prediction_horizon_days):
                ...
                if include_seasonality:
                    ...
                    seasonal_multiplier = [0.8, 1.0, 1.1, 1.0, 1.2, 0.9, 0.7][weekday]
