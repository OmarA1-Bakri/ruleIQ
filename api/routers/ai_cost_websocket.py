...
async def realtime_cost_dashboard(
    ...
    token: Optional[str] = Query(
        None, description="JWT token for authentication (deprecated - use headers)"
    ),
    ...
    try:
        ...
        logger.info(
            "User %s connected to real-time cost dashboard (%s)" % (user_id, dashboard_type)
        )
        while True:
            try:
                ...
                await websocket.send_text(
                    json.dumps(
                        {"type": "ping", "timestamp": datetime.now().isoformat(), "data": {}}
                    )
                )
...
async def send_service_initial_data(connection_id: str, service_name: str) -> None:
    ...
    try:
        ...
        service_data = {
            ...
            "average_cost_per_request": str(total_cost / total_requests)
            if total_requests > 0
            else "0",
            "daily_breakdown": [
                {
