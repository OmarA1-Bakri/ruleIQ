...
class EnforcementAnalyzer:
    ...
    def _load_database(self) -> Dict[str, Any]:
        ...
        if not self.db_path.exists():
            logger.info("Warning: Enforcement database not found at %s" % self.db_path)
    ...
    def _analyze_patterns(self) -> None:
        ...
        for pattern_tag, actions in pattern_groups.items():
            ...
            self.patterns_cache[pattern_tag] = EnforcementPattern(
                ...
                risk_multiplier=self._calculate_risk_multiplier(penalties, len(actions)),
    ...
    def _categorize_pattern(self, pattern_tag: str) -> str:
        ...
        categories = {
            "technical": ["technical_measures", "access_control", "security", "encryption"],
    ...
    def _calculate_risk_multiplier(self, penalties: List[float], frequency: int) -> float:
    ...
    def get_enforcement_evidence(
        self, regulation: str, violation_type: Optional[str] = None, sector: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for action in actions:
            ...
            if regulation.lower() in action.get("regulation", "").lower():
                score += 3
            if violation_type and violation_type in action.get("violation_category", ""):
                score += 2
        ...
        relevant_actions.sort(key=lambda x: (x["relevance_score"], x["date"]), reverse=True)
    ...
    def calculate_risk_adjustment(
        ...
        if evidence:
            avg_penalty = statistics.mean([e["penalty"] for e in evidence if e["penalty"] > 0])
        ...
        if business_context.get("serves_vulnerable_customers"):
            vulnerable_cases = [
                e for e in evidence if "vulnerable" in str(e.get("violation", "")).lower()
            ]
            if vulnerable_cases:
    ...
    def identify_compliance_gaps(
        ...
        for gap, frequency in gap_frequency.items():
            examples = [
                a["id"] for a in sector_actions if gap in a.get("compliance_gaps_identified", [])
            ][:3]
            gaps.append(
                {
                    "gap_id": gap,
                    "priority_score": frequency * 2,
    ...
    def _get_gap_remediation(self, gap: str) -> str:
        ...
        return remediations.get(gap, f"Implement controls to address {gap.replace('_', ' ')}")
    ...
    def get_regulatory_trends(
        ...
        for reg, actions in regulator_actions.items():
            ...
            avg_older = statistics.mean(older_penalties) if older_penalties else avg_recent
            ...
            trends.append(
                RegulatoryTrend(
                    ...
                    year_over_year_change=(avg_recent - avg_older) / avg_older
                    if avg_older > 0
                    else 0,
    ...
    def _predict_regulatory_focus(self, regulator: str, recent_actions: List[Dict]) -> List[str]:
        ...
        sorted_patterns = sorted(pattern_trajectory.items(), key=lambda x: x[1], reverse=True)
    ...
    def generate_enforcement_summary(
        ...
        return {
            ...
            "enforcement_evidence": evidence[:3],
            "risk_assessment": {
                "base_risk": base_risk,
            ...
            "regulatory_trends": [
                {"regulator": t.regulator, "direction": t.trend_direction, "focus": t.focus_areas}
                for t in trends[:3]
            ],
    ...
    def _generate_recommendations(self, evidence: List[Dict], gaps: List[Dict]) -> List[str]:
...
class IQEnforcementIntegration:
    ...
    def enhance_risk_assessment(
        ...
        return {
            ...
            "evidence_references": [e["case_id"] for e in summary["enforcement_evidence"]],
            ...
            formatted_evidence.append(
                {
                    ...
                    "summary": e["violation"],
                    "penalty": e["penalty"],
                    "url": f"https://regulator.gov.uk/enforcement/{e['case_id']}",
                }
            )
...
if __name__ == "__main__":
    ...
    for gap in gaps[:3]:
        logger.info("  - %s: Priority %s" % (gap["gap_id"], gap["priority_score"]))
